...
export function CommandPalette() {
  ...
  return (
    <div
      className="fixed inset-0 z-[60] flex items-start justify-center bg-white/40 px-4 pt-[12vh] backdrop-blur-sm"
    ...
    >
      ...
      >
        <div className="flex items-center gap-3 border-b border-stone-100 px-4 py-3">
          <Search className="h-4 w-4 text-stone-600" aria-hidden />
          <input
            ref={inputRef}
            value={q}
            onChange={(e) => setQ(e.target.value)}
            placeholder="Search pages, customers, actions…"
            className="flex-1 bg-transparent text-sm text-stone-900 placeholder:text-stone-600 focus:outline-none"
          />
          <kbd className="rounded border border-stone-200 bg-white px-1.5 py-0.5 font-mono text-[10px] text-stone-500">
            ESC
          </kbd>
        </div>
            ...
            (["Navigate", "Actions"] as const).map((g) => {
              ...
              return (
                <div key={g} className="mb-1">
                  <p className="px-3 pb-1 pt-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-stone-600">
                    {g}
                  </p>
                  {groups[g].map((c) => {
                    runningIdx += 1;
                    ...
                    return (
                      ...
                      >
                        ...
                        {c.hint ? (
                          <span className="text-[10px] text-stone-600">{c.hint}</span>
                        ) : null}
                        {isActive ? (
                          <ArrowRight className="h-3 w-3 text-stone-600" aria-hidden />
                        ) : null}
                      </button>
                    );
                  })}
        ...
        <div className="flex items-center justify-between border-t border-stone-100 px-4 py-2 text-[10px] text-stone-600">
          <span className="inline-flex items-center gap-1.5">
            <kbd className="rounded border border-stone-200 bg-white px-1 font-mono">↑↓</kbd>
            navigate
            <kbd className="rounded border border-stone-200 bg-white px-1 font-mono">↵</kbd>
            select
          </span>
          <span className="inline-flex items-center gap-1">
            <kbd className="rounded border border-stone-200 bg-white px-1 font-mono">⌘K</kbd>
            anywhere
          </span>
        </div>
      </div>
