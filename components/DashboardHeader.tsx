...
export function DashboardHeader({
  ...
  return (
    <header className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-stone-200 sm:p-7">
      ...
      <div className="mt-5 grid grid-cols-1 gap-6 sm:grid-cols-3 sm:divide-x sm:divide-stone-100">
        {stats.map((s, i) => (
          <div key={s.label} className={i > 0 ? "sm:pl-6" : ""}>
            ...
            <p className="mt-1.5 text-3xl font-semibold tracking-tight text-stone-900 tabular-nums">
              {s.value}
              {s.sub ? (
